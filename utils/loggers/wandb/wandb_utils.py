...
class WandbLogger():
    ...
    def val_one_image(self, pred, predn, path, names, im):
        ...
        :param pred: shape: torch.Size([当前图像nms最终筛完的预测框数量(不超过300),6])
                     6中0:4表示预测框坐标(x1, y1, x2, y2)-均为实际尺寸坐标(映射到yolo模型实际输入图像尺寸上(640,640)或(672,另一个可被32整除)
                     6中4表示当前的预测概率值
                     6中5表示当前的预测类别(0~79)
        :param predn: shape: torch.Size([当前图像nms最终筛完的预测框数量(不超过300),6])
                             6中0:4表示映射到原图尺寸的实际预测框坐标(x1,y1,x2,y2)
                             6中4表示当前的预测概率值
                             6中5表示当前的预测类别(0~79)
        :param path: Path(当前图片储存路径)
        :param names: 各类别映射字典，{0:'person', 1:'bicycle', ..., 79:'toothbrush'}
        :param im: shape: torch.Size([3,H,W]) 归一化后的当前图像(yolo模型实际输入图像尺寸(640,640)或(672,另一个可被32整除)
        :return:
        '''
        """
        ...
        if len(self.bbox_media_panel_images) < self.max_imgs_to_log and self.current_epoch > 0:
            # self.max_imgs_to_log=16，self.bbox_interval=30，每隔30个epoch进入一次，每次list中存入当前epoch前16张val图像预测结果用于展示
    ...
    def end_epoch(self, best_result=False):
        """
        将每隔30个epoch（大于0）的val图像预测结果存到wandb字典中 + 更新当前epoch全部字典值到wandb网页
    ...
    def finish_run(self):
        """
        展示当前epoch中wandb字典中存储的全部字段到wandb网页
        """
        if self.wandb_run:
            if self.log_dict:
                with all_logging_disabled():
                    wandb.log(self.log_dict) # dict
            wandb.run.finish()
